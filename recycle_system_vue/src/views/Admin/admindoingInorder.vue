<template>
  <div v-if="show">
    ...
    <el-table :data="tableData" :header-cell-style="{background:'transparent'}">
      ...
      <el-table-column label="派送员" width="200" align="center">
...
<script>
  export default {
    methods: {
      ...
      findbyrequire(){
        const _this = this
        axios.post('http://localhost:8181/adminfindAllDoingOrders/1/7',this.adminrequire).then(function (resp) {
      ...
      page (currentPage) {
        const _this = this
        axios.post('http://localhost:8181/adminfindAllDoingOrders/'+currentPage+'/7',this.adminrequire).then(function (resp) {
          _this.tableData = resp.data.list
          _this.pageSize = resp.data.pageSize
          _this.total = resp.data.total
        })
