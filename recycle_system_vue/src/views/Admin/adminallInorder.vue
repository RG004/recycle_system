      ...
      <el-table-column  label="评价" width="200">
        <template slot-scope="scope">
          <el-popover placement="right" width="400" trigger="click">
            ...
            <el-button  v-show="scope.row.evaluationId!=null&&scope.row.finishedTime!=null" type="primary" round slot="reference" @click="selectEvaluation(scope.row.evaluationId)">查看评价</el-button>
          </el-popover>
...
<script>
  export default {
    methods:{
      ...
      page(currentPage){
        const _this = this
        axios.post('http://localhost:8181/adminfindAllOrders/'+currentPage+'/4',this.adminrequire).then(function (resp) {
          _this.tableData=resp.data.list
          _this.pageSize = resp.data.pageSize
          _this.total = resp.data.total
        })
