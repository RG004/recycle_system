...
<script>
  export default {
    methods:{
      ...
      findbyrequire(){
        const _this = this
        axios.post('http://localhost:8181/adminfindAllOrders/1/7',this.adminrequire).then(function (resp) {
      ...
      page(currentPage){
        const _this = this
        axios.post('http://localhost:8181/adminfindAllOrders/'+currentPage+'/7',this.adminrequire).then(function (resp) {
          _this.tableData=resp.data.list
          _this.pageSize = resp.data.pageSize
          _this.total = resp.data.total
        })
