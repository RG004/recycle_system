<template>
  <div v-if="show">
    ...
    <el-button icon="el-icon-search" @click="findbyrequire" style="width: 80px">查询</el-button>
    ...
    <el-table :data="tableData" :header-cell-style="{background:'transparent'}">
      ...
      <el-table-column align="center" prop="scheduledTime" label="预约时间" width="200">
      </el-table-column>
      ...
      <el-table-column align="center" prop="userName" label="用户" width="100">
      </el-table-column>
      <el-table-column align="center" label="派送员" width="100">
        <template slot-scope="scope" >
...
<script>
  export default {
    methods:{
      ...
      findbyrequire(){
        const _this = this
        axios.post('http://localhost:8181/adminfindalldonate/1/7',this.adminrequire).then(function (resp) {
      ...
      page(currentPage){
        const _this = this
        axios.post('http://localhost:8181/adminfindalldonate/'+currentPage+'/7',this.adminrequire).then(function (resp) {
          _this.tableData=resp.data.list
          _this.pageSize = resp.data.pageSize
          _this.total = resp.data.total
        })
