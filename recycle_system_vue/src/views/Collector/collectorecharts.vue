...
export default {
  ...
  created () {
    ...
    axios.get('http://localhost:8181/collectorselectCountbydate/' + _this.$store.getters.getCollectorId ).then(function (resp) {
        console.log(resp)
    ...
    axios.get('http://localhost:8181/collectorselectCountbytime/' + _this.$store.getters.getCollectorId ).then(function (resp) {
        console.log(resp)
        for (var j = 0, len = resp.data.length; j < len; j++) {
          // alert(_this.x[j].recycleOrderId)
          vm.yData1.splice(j, 1, resp.data[j].count)
          vm.xData1.splice(j, 1, resp.data[j].scheduledTime+":00")
        }
      }
    )
    axios.get('http://localhost:8181/collectorselectAmountbydate/'+ _this.$store.getters.getCollectorId).then(function (resp) {
        console.log(resp)
        /*   Data=resp.data;
    ...
    axios.get('http://localhost:8181/collectorselectCountByitemid/'+_this.$store.getters.getCollectorId).then(function (resp) {
        console.log(resp)
        /* /!*Data=resp.data;
         alert(JSON.stringify(Data))*!/*/
        for (var j = 0, len = resp.data.length; j < len; j++) {
